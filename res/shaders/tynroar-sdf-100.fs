...
{
	vec2 pos = 
		vec2(mod(float(index), datasize.x) + 0.5, floor(float(index) / datasize.x) + 0.5)
			/
		datasize;
	pos.y = 1.0 - pos.y;
...
void main()
{
    ...
    for (float i = 0.0; i < datasize.x * datasize.y; i++)
    {
	...
	int datablockSize = int(baseargs[0] * 255.0);
	
	if ( datablockSize == 0 ) { break; }

	// --- sdf entity

	int shapetype = int(typeargs[0] * 255.0);
    ...
    // coloring
    vec3 colora = vec3(0.196,0.339,0.31);
    vec3 colorb = vec3(0.851,0.841,0.835);
    vec3 col = (d>0.0) ?  colora : colorb;
    //distance fade
    col *= 1.0 - exp(-6.0*abs(d));
    // onion radius
    col *= 0.6 + smoothstep(0.01,0.3,0.5*cos(100.0*d));
    // outline
    col = mix( col, vec3(1.0, 1.0, 0.961), 1.0-smoothstep(0.001,0.002,abs(d)) );

    gl_FragColor = vec4(col,1.0);
    //gl_FragColor = vec4(texture2D(dataset, fragCoord / resolution).xyz, 1.0);
}
